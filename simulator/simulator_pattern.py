    ...
    def __init__(self, **kwargs):
        ...
        if self.simulator_mode == 'toy_mode':
            ...
            if env_params['premium_taxi_mode'] == True:
                ...
                driver_sample_size_all = self.sample_driver_size # 9402 for full data
                drivers_to_keep = random.sample(list(all_drivers), driver_sample_size_all)
                print(f"keeping {driver_sample_size_all }unique driver_id")
                self.normal_driver_info = self.driver_info[self.driver_info['driver_id'].isin(drivers_to_keep)]
                # self.driver_info = self.driver_inf
