...
class Simulator:
    def __init__(self, **kwargs):
        ...
        self.sample_driver_size = kwargs['driver_num']
        pattern_params = {'simulator_mode': self.simulator_mode, 'request_file_name': self.request_file_name,
                          'driver_file_name': self.driver_file_name, 'sample_driver_size': self.sample_driver_size}
    ...
    def step_bootstrap_new_orders(self, score_agent={}, epsilon=0): #  rl for matching
        ...
        # TJ
            ...
            if len(sampled_requests) > 0:
                ...
                wait_info.loc[(wait_info['trip_distance'] <= 2),['maximum_price_passenger_can_tolerate']] = skewed_normal_distribution_3_param(price_params_dict['short'][0],price_params_dict['short'][1],price_params_dict['short'][2],short_wait)
                wait_info.loc[(wait_info['trip_distance'] > 2) & (wait_info['trip_distance'] <=5 ),['maximum_price_passenger_can_tolerate']] = skewed_normal_distribution_3_param(price_params_dict['short_medium'][0],price_params_dict['short_medium'][1],price_params_dict['short_medium'][2],short_medium)
...
# load pickup time
    ...
    def update_time(self):
        ...
        self.current_step = int((self.time - self.t_initial) // self.delta_t)
        self.current_step_in_hour = self.current_step % int(3600 / self.delta_t) # step in hour
        self.hour = self.time // 3600 # hour in day
        # rl for matching
